...
public class TransferAnalysisApiController {
    ...
    public Optional<List<TransferSummary>> readFromToAnalysis(@PathVariable("site") String site,
        ...
        System.out.println(
            String.format("Request API: readFromToAnalysis, Input Params: %s, %s, %s", site, year,
                month));

        Optional<TransferAnalysisService.FromToMatrix> fromToMatrix = transferAnalysisService.readFromToMatrixBy(
            site, year, month);
